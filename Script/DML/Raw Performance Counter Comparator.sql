-- Performs the snapshots and calculations automatically, allowing the output to be analyzed directly. 
-- There are other performance counters, not considered below, which have a  secondary, associated base counter by which the main counter has to be divided to arrive at its actual value.
